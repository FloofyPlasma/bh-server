...
{
  return self->voiceChat != NULL ? YES : NO;
}
...
- (void)match:(BHMatch*)match player:(NSString*)playerID didChangeState:(BHPlayerConnectionState)state
{
  ...
  if ([self->match respondsToSelector:@selector(usesGameCenterPlayerInformation)] && [self->match usesGameCenterPlayerInformation]) {
    if (playerID != NULL && [self->delegate respondsToSelector:@selector(postGCMatchInvitePlayers:)]) {
...
- (void)cleanup
{
  ...
  [self->match setDelegate:NULL];
  [self->match disconnect];
  self->livePlayerInfos = NULL;
  self->playerInfosForAllPlayersByID = NULL;
  self->voiceChat = NULL;
}
  ...
  for (NSMutableDictionary* playerInfo in self->livePlayerInfos) {
    ...
    for (NSDictionary* previousInfo in previousPlayerList) {
      if ([[previousInfo objectForKey:@"playerID"] caseInsensitiveCompare:playerID] == NSOrderedSame) {
        if ([previousInfo objectForKey:@"photo"] != NULL) {
  ...
  if (self->match != match_) {
    ...
    if ([self audioChatSupported]) {
      if ([[NSUserDefaults standardUserDefaults] boolForKey:@"audioChatDisabled"] == NO) {
        [self performSelector:@selector(startVoiceChat) withObject:NULL afterDelay:5];
      }
    }
